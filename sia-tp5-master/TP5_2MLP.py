...
encoder = MLP(n_input=35, n_hidden=HIDDEN, n_output=LATENT_DIM,
              activation_function=ACT_FUNC, learning_rate=LR, optimizer=OPTIMIZER)
decoder = MLP(n_input=LATENT_DIM, n_hidden=HIDDEN, n_output=35,
              activation_function=ACT_FUNC, learning_rate=LR, optimizer=OPTIMIZER)
...
for epoch in range(1, EPOCHS + 1):
    ...
    for batch in batches:
        ...
        z = encoder.forward(x_noisy)
        recon = decoder.forward(z)

        # Backward: backpropagation primero en decoder, luego en encoder
        decoder_grad_z = decoder.backward(z, y=x, output=recon)   # actualiza decoder, devuelve dL/dz
        encoder.backward(x_noisy, grad_output=decoder_grad_z)     # actualiza encoder usando dL/dz
...
#  Latent space y reconstrucciones
z_np = encoder.predict(data_noisy)
probs = decoder.predict(z_np)
...
gen_probs = decoder.predict(grid)
